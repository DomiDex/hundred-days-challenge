...
import { createMockRequest, expectFeedResponse } from '@/test-utils/api'
...
describe('GET /api/feed/atom', () => {
  ...
  describe('Error Handling', () => {
    it('should handle feed generation errors', async () => {
      ...
      expect(console.error).toHaveBeenCalledWith('Error generating Atom feed:', expect.any(Error))
    })
  ...
  describe('Feed Content', () => {
    it('should use atom content from generateFeeds', async () => {
      const customAtom =
        '<?xml version="1.0"?><feed xmlns="http://www.w3.org/2005/Atom"><title>Custom Atom Feed</title></feed>'
      ;(generateFeeds as jest.Mock).mockResolvedValue({
        rss: '',
        atom: customAtom,
        json: '',
