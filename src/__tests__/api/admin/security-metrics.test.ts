...
describe('GET /api/admin/security-metrics', () => {
  ...
  describe('Authentication', () => {
    ...
    it('should reject non-admin Bearer tokens', async () => {
      const token = generateAuthToken({ role: 'user' })
  ...
  describe('Severity Filtering', () => {
    it('should filter events by severity', async () => {
      ...
      expect(json.recentEvents).not.toContainEqual(
  ...
  describe('Export Functionality', () => {
    it('should support CSV export format', async () => {
      ...
      expect(response.headers.get('Content-Disposition')).toMatch(/attachment.*security-metrics.*\.csv/)
    })
    ...
    it('should support JSON export with download headers', async () => {
      ...
      expect(response.status).toBe(200)
      expect(response.headers.get('Content-Disposition')).toMatch(/attachment.*security-metrics.*\.json/)
    })
  })
